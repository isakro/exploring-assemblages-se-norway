...
lithics2 <- sites %>% 
  ...
  mutate(chip_prop =  sum(chip, chip_nonflint) / artefact_count,
         ...
         blanks_prop = sum(blade, microblade, flake, flake_nonflint, fragment,
            fragment_nonflint, blade_nonflint,  microblade_nonflint) / artefact_count,
...
wabicorflint <- cor(log10(wabiflint$flint_dens), log10(wabiflint$secondary_prop))
...
lithicstr <- lithics2 %>%
  ...
  relocate("mean_date",
    "log10(lithic_dens)", "nonflint_prop^(1/3)", "blanks_prop", "log10(secondary_prop)",
    "chip_prop", "core_prop^(1/3)" 
  )
...
# Reviewer 2 wanted non-flint as a separate column and the artefact categories
# combined, irrespective of raw-material. 
...
caallcats <- CA(select(all_cats, -site, -mean_date, -artefact_count, -flint_count, 
                       -nonflint_count, -reported_start_bc, -reported_end_bc,
                       -askeladden_id),
                graph = FALSE)
