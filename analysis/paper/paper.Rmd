...
The results of the correspondence analysis does appear to align reasonably well with previous research that is based on more informal analyses of, indicating that the employed artefact categories are behavourally meaningful. Aligning the signficance of this chronological patterning to specific behavioural dimensions, is, however, another task entirely, and a task that is severely hampered by fact that for the most part we lack even a most basic understanding of what any individual lithic object has been used for, and consequently how the different debitage and tool categories should be treated @dibble2017. For example, a vast amount of artefacts defined as debitage are likely to have fulfilled the function of tools, and both debitage and formal tools could have had various different purposes and had a multitude of shapes throughout their use-life. Reducing the number of such inferential leaps by aggregating artefact groups could potentially subsume important variation, but it does also reduce the possibility that the conclusions of any analysis is not simply the result of the employed units of analysis and an overly naive trust in the fact that these units reflect 
