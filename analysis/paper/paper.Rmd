...
```{r variable-aggregation, fig.cap = "Aggregation of variables for the correspondence analysis. The column on the left shows the data as originally compiled. The column on the right shows how these have been aggregated for the analysis.", fig.height = 7}
...
sn <- sankeyNetwork(Links = links, Nodes = nodes,
              ...
              NodeGroup="group", nodeWidth = 12, nodePadding = 7, fontSize = 12)
...
```{r, fig.height = 15, fig.cap = "Same as above, only that here the proportion of non-flint is used as a supplementary column. The negligble difference between this CA and that above indicates that the raw material variability is integrated in the different artefact types, and therefore that the effect of artefact types and raw material cannot be seperated. These plots thus hide important variability that is captured in the ones presented in the main text. A) Row plot, B) Column plot."}

# Version of the above where the proportion of non-flint is simply added as a 
# supplementary column, followng Reviewer 2. 
...
splt4 <- fviz_ca_col(nonpropsup, col.col.sup = "black", col.col = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), repel = TRUE,
             title = "B") +
  labs(colour = "Contribution to total inertia") +
  theme(legend.position = "bottom")
