...
# for colouring points in the maps). Set up labels to aggregate. There might be
...
sitejoin <- right_join(site_pts, sites, by = c("LOKID" = "askeladden_id")) %>% 
  mutate(no = as.numeric(no)) %>% 
    mutate(no_m = ifelse(no %in% c(1:10, 20) & !duplicated(no %in% c(1:10, 20)),
                         "1-10, 20",
           #ifelse(no %in% c(7, 9:10, 20) & !duplicated(no %in% c(7, 9:10, 20)), "7, 9-10, 20",
           ifelse(no %in% 11:12 & !duplicated(no %in% 11:12), "11-12", 
           ifelse(no %in% 14:17 & !duplicated(no %in% 14:17), "14-17", 
           ifelse(no %in% 21:22 & !duplicated(no %in% 21:22), "21-22", 
           ifelse(no %in% 25:27 & !duplicated(no %in% 25:27), "25-27",
           ...
           ifelse(no %in% c(32:33, 35) & 
                ...
                "32-33, 35",
           #ifelse(no %in% 32:33 & !duplicated(no %in% 32:33), "32-33",
           ifelse(no %in% c(24, 34, 37:40)  &
                    !duplicated(no %in% c(24, 34, 37:40)), 
                  "24, 34, 37-40",
            ifelse(no %in% c(42:43, 45:46, 53)  &
                    !duplicated(no %in% c(42:43, 45:46, 53)), 
                  "42-43, 45-46, 53",
           #ifelse(no %in% 42:43 & !duplicated(no %in% 42:43), "42-43",
           #ifelse(no %in% 45:46 & !duplicated(no %in% 45:46), "45-46",
           ifelse(no %in% 48:50 & !duplicated(no %in% 48:50), "48-50",
           ifelse(no %in% c(31, 47, 51:52) & !duplicated(no %in% c(31, 47, 51:52)), "31, 47, 51-52",
           ifelse(no %in% c(13, 30, 36,
                            41, 44, 54), no, "")))))))))))))
