...
rcarb_data <- do.call(rbind, posteriors) 
  group_by(site) %>%
  mutate(meandate = mean(date_grid * -1)) %>% 
  ungroup() %>% 
  arrange(meandate)

h3 <- radiocarbon %>% 
  filter(site == "Vallermyrene 1" | site == "Stokke/Polland 5") 
...
my_result_data <- parseOxcalOutput(my_result_text, only.R_Date = FALSE)
plot(my_result_data$`Vallermyrene 1`$posterior_probabilities$dates, my_result_data$`Vallermyrene 1`$posterior_probabilities$probabilities, type = "l")

posterior <- data.frame(probabilities = my_result_data$`Vallermyrene 1`$posterior_probabilities$probabilities,
                        date_grid = my_result_data$`Vallermyrene 1`$posterior_probabilities$dates,
                        site = "Vallermyrene 1",
                        excavation_project = "Vestfoldbaneprosjektet")

ggplot() +
  geom_line(aes(x = yrs, y = my_result_data$` Vallermyrene 1 `$posterior_probabilities$probabilities))


yrs <- seq(11500, 5000, -500)
plt1 <- ggplot(sites, aes(x = yrs)) +
  geom_segment(aes(x = reported_start_bc, xend = reported_end_bc,
                   yend = reorder(site, mean_date), y = reorder(site, mean_date),
...
ggplot(sites, aes(x = yrs)) +
      ggridges::geom_ridgeline(data = rcarb_data, aes(x = date_grid * -1,
                                                      y = fct_reorder(site, date_grid * -1, .fun = mean),
                                      height = probabilities * 100,
                                      fill = excavation_project)) +
    geom_segment(aes(x = reported_start_bc, xend = reported_end_bc,
                   yend = reorder(site, mean_date), y = reorder(site, mean_date),
                   colour = excavation_project), size = 1.7) +
  xlab("Cal. BC") +
  ...
  guides(colour =  guide_legend(title.position = "top", title.hjust = 0.5)) +
    ...
    ggridges::geom_ridgeline(data = rcarb_data, aes(x = date_grid * -1,
                                      y = fct_reorder(as.factor(site), date_grid * -1, .fun = mean),
                                      height = probabilities * 100,
                                      group = fct_reorder(as.factor(site), date_grid * -1, .fun = mean),
                                      fill = excavation_project)) +
  scale_fill_manual(labels = c("E18 Rugtvedt-Dørdal", 
                                "Vestfoldbaneprosjektet", "E18 Bommestad-Sky"),
                     values = c("springgreen4",
                                "gold1", "firebrick3"),
                    guide = FALSE) +

pst <- get_posterior_probabilities(my_result_data)
