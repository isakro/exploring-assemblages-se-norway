...
```{r dist, fig.cap = "A) Spatial and B) temporal distribution of the sites chosen for analysis. Radiocarbon age determintations are given as the sum of the posterior density esimates. Solid lines indicate that the site has been dated with reference to relative sea-level change and typological indicators. These follow the original reports.", fig.height = 13, fig.width = 12}
...
```{r ldist, fig.cap = "A) Spatial and B) temporal distribution of the sites chosen for analysis. Radiocarbon age determintations are given as the sum of the posterior density esimates. Solid lines indicate that the site has been dated with reference to relative sea-level change and typological indicators. These follow the original reports. Site names corresponding to numbers are given in Table 1.", fig.height = 9, fig.width = 12, eval = FALSE}
temp_dist
```
...
sites %>% 
  ...
  gt() %>% 
    ...
    strt_bce = "Reported start (BCE)",
    end_bce = "Reported end (BCE)",
  ) %>% 
  tab_header(
    title = "Table 1. Analysed sites."
  ) %>% 
...
```{r cor, fig.cap = "Correspondence analysis using the artefact count data. A) Object map, B) Variable map.", fig.height = 13}
# Plot sites
...
lithics2 <- sites %>% 
  select(-site, -beach_flint_flint_nodule, -hammerstone, -excavation_project,
          -askeladden_id) %>% 
...
```{r cor-plot, fig.cap = "Correlation matrix showing transformation of skewed variables for the PCA, and the linear correlation between these. The mean age of the sites has also been included to visualise overall temporal trends.", fig.height = 9.5, fig.width = 10}
