...
lithics3 <- gisjoin %>% 
  ...
  mutate(chip_freq =  sum(chip, chip_other) / artefact_count,
         ...
         nonflint_freq = nonflint_count / artefact_count,
         lithic_dens = artefact_count / excavated_m3,
         viewsize_5k = log10(viewsize_5k),
         viewsize_10k = log10(viewsize_10k),
         fetch = log10(avg_fetch)) %>%
  select(-blanks_lithics, -secondary_lithics, -excavated_m3, -artefact_count,
         -flint_count, -nonflint_count, -core, -core_fragment, -chip, 
         -chip_other, -core_other, -core_fragment_other, -cat, -municipality, -quality,
         -elev_minimum, -site_size, -cent_elev, -phase, -slope_average, -aspect_average,
         -infiltration_mode,-emergence_50, -emergence_500, -emergence_1k, -location,
         -island_size) %>% 
  column_to_rownames("site")
