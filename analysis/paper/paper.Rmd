...
institute:
  - UiO: University of Oslo, Department of Archaeology, Conservation and History
...
keywords: |
  Mesolithic Scandinavia; Multivariate statistics; Mobility strategies; Whole Assemblage Behavioural Indicators
---
...
* Multivariate exploratory analysis of Mesolithic assemblages in south-eastern Norway
* Explores patterns related to established artefact categories in Norwegian archaeology
* Explores variables associated with mobility patterns in lithic assemblage studies
* Draws on the Whole Assemblage Behavioural Indicators (WABI)
* Relevance for Mesolithic Norway supports the notion that WABI are widely applicable
...
source(here::here("analysis/script/plot_functions.R"),
...
sites <- assemblages %>% 
  ...
  mutate(
    ...
    artefact_count = sum(c_across(core:flake_retouch_nonflint)),
    flint_count = sum(c_across(core:flake_retouch)),
    nonflint_count = sum(c_across(axe_nonflint:flake_retouch_nonflint))) %>%
...
```{r dist, fig.cap = "A) Spatial and B) temporal distribution of the sites chosen for analysis. Radiocarbon age determinations are given as the sum of the posterior density esimates. Solid lines indicate that the site has been dated with reference to relative sea-level change and typological indicators. These follow the original reports. Site numbers match those provided in Table 2.", fig.height = 13, fig.width = 12}
...
sitemap <-
  ...
  ggsn::north(sitejoin, scale = 0.2, symbol = 10,
              anchor = c(x = anc[2] - 30000, y = anc[1]) + 5500) +
...
The lithic data analysed here is based on the classification of the site assemblages done for the original excavation reports, and consists of `r ncol(select(sites, core:flake_retouch_nonflint))` debitage and tool types. These represent artefact categories that have been used consistently across the reports. Consequently, sub-categories that have only been used in the classification of some inventories have been omitted. This for example pertains to what blanks have been used for the production of formal tools, which has only been noted in some of the reports. Furthermore, the artefact data have been divided into flint and non-flint materials. Flint does not outcrop naturally in southern Norway, and is only available locally as nodules that have been transported and deposited by retreating and drifting ice [e.g. @berg-hansen1999]. This means that the distribution and quality of flint has been impacted by a diverse set of climatic and geographical factors [@eigeland2015, 46]. Thus, while flint is treated as a unified category here, the variability in quality could have been substantial. Furthermore, the various non-flint raw materials that have been lumped together have quite disparate properties, where fine-grained cryptocrystalline materials are often used as a substitute or supplement to flint, while other, coarser materials are usually associated with the production of axes and other macro tools. Given this differentiated use, these raw-material properties are expected to be reflected in the retained debitage and tool categories. An important benefit of combining all of the non-flint materials is that this reduces the dependency on whether or not these have been correctly and consistently categorised for the reports [cf. @frivoll2017]. Finally, while factors such as landscape changes through shoreline displacement can have led to variable raw-material availability at the analysed sites, for example by impacting accessibility by means of watercraft, the relatively constrained geographical distribution of the sites hopefully counteracts some environmentally given sources of variation.
...
sites %>% 
  mutate(strt_bce = reported_start_bc * -1, 
         ...
         dating_method = ifelse(dating_method == "shore_typo",
                                "Shoreline/typology", "Radiocarbon")) %>% 
...
links <- data.frame(value = 1,
        categories = names(sites %>% select(core:flake_retouch_nonflint))) %>% 
  mutate(new_categories = ifelse(categories %in% c("microlith", "tanged_point",
                          ...
                          ifelse(categories %in% c("axe", "chisel"),
                                 "macro_tool", 
                          ifelse(categories %in% c("axe_nonflint", "chisel_nonflint",
                                 "club_hatchet_nonflint"), "macro_tool_nonflint", 
                          ifelse(categories %in% c("knife_nonflint", 
                                 "scraper_nonflint", "drill_nonflint", 
                                 "microblade_retouch_nonflint", 
                                 "blade_retouch_nonflint", 
                                 "fragment_retouch_nonflint",
                                 "flake_retouch_nonflint"), "small_tool_nonflint",
                                 paste0(categories, " ")))))))
...
my_colour <-'d3.scaleOrdinal() .domain(["core", "core_fragment", "blade", "microblade", "chip", "fragment", "flake", "axe_debitage", "macro_tool", "projectile", "microburin", "small_tool", "beach_flint_flint_nodule", "macro_tool_nonflint", "axe_debitage_nonflint", "grinding_slab_stone", "projectile_nonflint", "small_tool_nonflint", "hammerstone", "flake_nonflint", "fragment_nonflint", "chip_nonflint", "blade_nonflint", "microblade_nonflint", "core_nonflint", "core_fragment_nonflint,"]) .range(["lightgrey", "lightgrey", "lightgrey", "lightgrey", "lightgrey", "lightgrey", "lightgrey", "lightgrey", "#ff8086", "#79d49f", "lightgrey", "#8eb8fa", "lightgrey", "#a88c7d", "lightgrey", "lightgrey", "lightgrey", "#ffee8c",  "lightgrey", "lightgrey", "lightgrey", "lightgrey", "lightgrey", "lightgrey", "lightgrey", "lightgrey"])'
...
As these measures are argued to predominantly be determined by land-use and mobility patterns, relative frequency of chips and relative frequency of non-flint material are also included in the analysis, as these measures have also been linked to mobility patterns [e.g. @Bicho2020; @kitchel2021] and are of central importance to Norwegian Stone Age archaeology [e.g. @breivik2016a; @reitan2016]---the use of local non-flint material has been taken to indicate reduced mobility and increased familiarity with local surroundings [@jaksland2001, 112; @glørstad2010, 181].

# Methodology

The exploratory approach taken here means that a wide range of combinations and transformations of variables has been explored to identify patterning in the data. While only parts of this process can sensibly be reported upon, all data and employed R programming scripts [@rcoreteam] are freely available as a research compendium at <https://osf.io/ehjfc/>, following @marwick2018b, allowing readers to explore and scrutinise the data and the final analytical choices made [@Marwick2017].
...
The first part of the analysis involves employing the method of correspondence analysis (CA), using the lithic count data as classified for the original excavation reports [e.g. @baxter1994; @shennan1997]. As this part of the analysis partially draws on the above-mentioned Frison effect, several artefact categories have been collapsed for the CA. A version of the CA using the original artefact categories, as well as some additional configurations and ways to aggregate the variables are also available in the supplementary material to the paper.

Following the WABI and other factors associated with mobility patterns, as presented above, the variables employed in the second part of the analysis are relative frequency of secondarily worked lithics (RFSL), defined as the proportion of the assemblages constituted by retouched or ground lithics; volumetric density of lithics (VDL), defined as the total number of artefacts divided by total excavated m³ as taken from the original reports; relative frequency of chips, defined as the proportion of artefacts with size \< 1mm; relative frequency of cores, the proportion of all artefacts classified as cores in the original reports; relative frequency of blanks, here defined as the proportion of all artefacts classified as flakes, blades, micro-blades or fragments; and finally relative frequency of non-flint material. Following @Bicho2020, the analysis is done using principal components analysis (PCA), leading to a shift in focus from the relative composition emphasised by the CA, to having more weight placed on patterning in the most abundant occurrences [@baxter1994, 71--77].

A note should also be made on the fact that a few variables that are sometimes invoked for the classification of sites in terms of associated mobility patterns are omitted here [e.g. @bergsvik1995, 116; @Bicho2020; @breivik2016a]. For the assemblage data itself this especially pertains to diversity in tool-types [@canessa2021], which has been omitted in light of the above-metioned Frison effect. Number of features on the sites has also been disregarded as taphonomic loss is likely to have led to a chronological bias in their preservation. Similarly, the number of activity areas, effectively number of artefact clusters, however defined, has also been disregarded. This follows most notably from the fact that the impact of post-depositional processes at Stone Age sites in Norway is arguably understudied [@jørgensen2017]. This pertains for example to bio-turbation in the form of three-throws, which can have a detrimental effect on the original distribution of artefacts, and which can be expected to have impacted several of the sites treated here [@jørgensen2017; @darmark2018e].
...
lithics <- sites %>%
  ...
  mutate(
    ...
    macro_tool_nonflint = axe_nonflint + chisel_nonflint + club_hatchet_nonflint,
    small_tool_nonflint = knife_nonflint + scraper_nonflint + drill_nonflint + 
      microblade_retouch_nonflint + blade_retouch_nonflint + fragment_retouch_nonflint +
      flake_retouch_nonflint,
    .keep = "unused") %>%
...
flint <- lithics %>% select(!(ends_with("nonflint")), -grinding_slab_stone, -hammerstone)
```
...
# Plot sites
plt2 <- fviz_ca_row(calithics, col.row = "black",
                    fill = lithics$mean_date / 1000, title = "B",
...
plt3 <- fviz_ca_col(caflint, col.col = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), repel = TRUE,
             title = "A") +
  labs(colour = "Contribution to total inertia") +
  theme(legend.position = "bottom") 

# Sites for the flint data
plt4 <- fviz_ca_row(caflint, col.row = "black", fill = lithics$mean_date / 1000,
                    title = "B",
                     repel = TRUE, shape = 21, pointsize = 2.9) +
  scale_fill_gradient2(name = "Mean age kyr BCE", low="yellow", mid="orange", 
                        high="red", midpoint = centdate) +
...
```{r flintcor, fig.height = 13, fig.cap = "Correspondence analysis using the flint data. A) Column plot, B) Row plot."}
plt3 /
plt4 
```
...
blanks_lithics <- c(blanks_flint, "flake_nonflint", "fragment_nonflint", 
                     "blade_nonflint",  "microblade_nonflint")
...
secondary_lithics <- c(secondary_flint, "axe_nonflint", "axe_debitage_nonflint",
                       "chisel_nonflint", "club_hatchet_nonflint", 
                       "grinding_slab_stone", "projectile_nonflint", 
                       "knife_nonflint", "scraper_nonflint", "drill_nonflint", 
...
lithics2 <- sites %>% 
  ...
  mutate(chip_prop =  sum(chip, chip_nonflint) / artefact_count,
         core_prop = sum(core, core_nonflint) / artefact_count,
         blanks_prop = sum(blade, microblade, flake, flake_nonflint, fragment,
            fragment_nonflint, blade_nonflint,
            microblade_nonflint) / artefact_count,
         secondary_prop =  sum(axe_debitage, axe, chisel, 
                ...
                scraper_nonflint, drill_nonflint, microblade_retouch_nonflint, 
                blade_retouch_nonflint, fragment_retouch_nonflint,
                flake_retouch_nonflint) / artefact_count,
  ...
  select(-blanks_lithics, -secondary_lithics, -excavated_m3, -artefact_count,
         ...
         -chip_nonflint, -core_nonflint, -core_fragment_nonflint) %>% 
...
flint2 <- sites %>% 
  ...
  select(!ends_with("nonflint")) %>% 
  mutate(chip_prop =  sum(chip) / artefact_count,
         core_prop = sum(core) / artefact_count,
         blanks_prop = sum(blade, microblade, flake, fragment) / artefact_count,
         secondary_prop =  sum(axe_debitage, axe, chisel, 
                microlith, tanged_point, transverse_point, single_edged_point,
                hognipen_point, projectile_undefined, microburin, scraper, 
                knife, burin, drill, blade_retouch, microblade_retouch, 
...
wabicor <- cor(log10(lithics2$lithic_dens), log10(lithics2$secondary_prop))

# Langangen Vestgård 2 does not have any retouched flint
wabiflint <- filter(flint2, secondary_prop > 0)
wabicorflint <- cor(log10(wabiflint$flint_dens),
                    log10(wabiflint$secondary_prop))

# Correlation with mean site date, all lithics
secfreqcor <- cor(lithics2$mean_date, lithics2$secondary_prop)
denscor <- cor(lithics2$mean_date, lithics2$lithic_dens)

# Correlation with mean site date, flint
secfreqcorflint <- cor(wabiflint$mean_date, wabiflint$secondary_prop)
denscorflint <- cor(wabiflint$mean_date, wabiflint$flint_dens)

```

```{r cor-plot, fig.cap = "Correlation matrix showing transformation of skewed variables for the PCA. The mean age of the sites has also been included to visualise overall temporal trends. Cells below the diagonal display the bivariate distributions with a fitted OLS-regression. The cells above the diagonal display and are coloured by the corresponding Pearson's correlation coefficient.", fig.height = 10.1, fig.width = 10.1}
...
lithicstr <- lithics2 %>%
  ...
  mutate(
         ...
         "log10(secondary_prop)" = log10(secondary_prop),
         "core_prop^(1/3)" = core_prop^(1/3),
         "nonflint_prop^(1/3)" = nonflint_prop^(1/3), .keep = "unused") %>% 
  relocate("mean_date",
    "log10(lithic_dens)", "nonflint_prop^(1/3)", "blanks_prop",
    "log10(secondary_prop)", "chip_prop", "core_prop^(1/3)" 
  )

corr_plot(select(lithicstr, -reported_start_bc, -reported_end_bc, -dating_method))
```

```{r pca-analysis, fig.cap = "PCA biplot resulting from analysing variables that have been related to mobility patterns. Note that details on the transformation of the variables has been left out of the plot for clarity, but follow those given in Figure 5.", fig.height = 8}
...
lithicspca <- lithicstr %>% 
  rename(
   ...
   secondary_prop = "log10(secondary_prop)",
   core_prop = "core_prop^(1/3)",
   nonflint_prop = "nonflint_prop^(1/3)"
  )


# Perform PCA with standardised variables (scale.unit = TRUE)
...
```{r wabi, fig.cap = paste0("Relative frequency of secondarily worked lithics plotted against the volumetric density of artefacts for A) All lithics (\\textit{r} = ", round(wabicor, 2), "), B) Flint (\\textit{r} = ", round(wabicorflint, 2), "). The logarithm is taken to base 10 on all axes."), fig.height = 5, fig.width = 10}

w1 <- ggplot(lithics2, aes(lithic_dens, secondary_prop * 100, 
                     fill = mean_date / 1000)) + 
...
w2 <- 
  ggplot(flint2, aes(flint_dens, secondary_prop * 100, 
                     fill = mean_date / 1000)) + 
  geom_point(shape = 21, size = 3) +
  geom_smooth(method = "lm", col = "black", se = FALSE) +
  labs(title = "B", 
...
```{r index}
# Create curation index using min-max normalisation

# First normalise the two variables
minmax <- lithics2 %>%  
  mutate(dens =  ((lithic_dens - min(lithic_dens)) / (max(lithic_dens) - min(lithic_dens))),
         secondary = ((secondary_prop - min(secondary_prop)) / (max(secondary_prop) - min(secondary_prop))))

minmaxflint <- flint2 %>% 
  mutate(dens =  ((flint_dens - min(flint_dens)) / (max(flint_dens) - min(flint_dens))),
         secondary = ((secondary_prop - min(secondary_prop)) / (max(secondary_prop) - min(secondary_prop))))

# Then find the mean while making dens negative
indexdata <- minmax %>% 
  rowwise() %>% 
...
sample_date <- function(site){
  ...
  # If not, draw a random year from the reported date range (uniform probability)
    ...
    sdate <- sample(
      as.integer(
        site["reported_start_bc"]):as.integer(site["reported_end_bc"]), 1)
  }
...
While there are certainly nuances in the material that might lead one to question the applicability of the VDL and RFSL measures for any individual site, the overall pattern for curation does appear robust. The curation index is relatively high and uniform until some time before 8000 BCE. This corresponds well with the view that the Early Mesolithic is characterised by a high and uniform degree of mobility. This is followed by a marked increase in expedience, which has stabilised by around 7000 BCE. Again, this corresponds well with the employed chronological framework. Referring back to the demographic changes that are to take place around this transition, the Microlith phase could thus represent a period where migrating people and new living practices were propagating through societies in south-eastern Norway---a process that in light of the curation data would have concluded around 7000 BCE.
...
As it stands, the main hypotheses resulting from the present analysis would be that settlement patterns in the earliest parts of the Mesolithic were characterised by relatively high and uniform degrees of mobility, which then drop before levelling off at around 7000 BCE. These then remain stable throughout the rest of the period, despite variation pertaining to other aspects of the lithic inventories, as evidenced by the CA. The fall in curation levels and parallel increase in variation would seem to correlate well with a transition from a predominantly residential to logistical settlement system [@binford1980]. This indicates, in turn, that the measures represent an empirical link between technological organisation and economic behaviour and mobility patterns [@riel-salvatore2004]. 
...
```{r supplementary1, fig.height = 18, fig.cap = "Correspondence analysis using all original artefact categories. A) Column plot, B) Row plot.", fig.width = 10}
all_cats <- sites %>%
  # Make site names the rownames
  column_to_rownames(var = "no") %>% 
  # Deselect a few of the columns not be used
...
caallcats <- CA(select(all_cats, -site, -mean_date, -artefact_count,
                       -flint_count, -nonflint_count, -reported_start_bc,
                       -reported_end_bc, -askeladden_id),
                graph = FALSE)

splt1 <- fviz_ca_col(caallcats, col.col = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), repel = TRUE,
             title = "A") +
  labs(colour = "Contribution to total inertia") +
  theme(legend.position = "bottom") 

splt2 <- fviz_ca_row(caallcats, col.row = "black",
            fill = all_cats$mean_date / 1000, title = "B",
                     repel = TRUE, shape = 21, pointsize = 2.9) +
  scale_fill_gradient2(name = "Mean age kyr BCE", low="yellow", mid="orange", 
                        high="red", midpoint = centdate) +
  theme(legend.position = "bottom") 
...
```{r supplementary2, fig.height = 15, fig.cap = "Correspondence analysis collapsing artefact types irrespective of raw-material and including proportion of non-flint as its own variable.  A) Column plot, B) Row plot."}

# Based on suggestion from reviewer: artefact types combined irrespective of 
# raw-material and  non-flint as supplementary column. I start by including 
# it as a normal column.
all_nonflint_prop <- sites %>%
    mutate(
    projectile = microlith + tanged_point + transverse_point + 
    single_edged_point + hognipen_point + projectile_undefined + 
      projectile_nonflint,
    small_tool = scraper + knife + burin + drill + blade_retouch + 
      ...
      fragment_retouch_nonflint +
      flake_retouch_nonflint,
    macro_tool = axe + chisel + axe_nonflint + 
      chisel_nonflint + club_hatchet_nonflint,
    core = core + core_nonflint,
    core_fragment = core_fragment + core_fragment_nonflint,
    axe_debitage = axe_debitage + axe_debitage_nonflint,
    blade = blade + blade_nonflint,
    microblade = microblade + microblade_nonflint,
    fragment = fragment + fragment_nonflint,
    flake = flake + flake_nonflint,
    chip = chip + chip_nonflint,
    nonflint_prop = nonflint_count / artefact_count,
    .keep = "unused") %>% 
    # Make site names the rownames
    column_to_rownames(var = "no") %>% 
...
nonprop <- CA(select(all_nonflint_prop, -site, -mean_date, -flint_count, 
                       -reported_start_bc, -reported_end_bc,
                       -askeladden_id),
                graph = FALSE)

splt3 <- fviz_ca_col(nonprop, col.col = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), repel = TRUE,
             title = "A") +
  labs(colour = "Contribution to total inertia") +
  theme(legend.position = "bottom") 

splt4 <- fviz_ca_row(nonprop, col.row = "black",
            fill = all_nonflint_prop$mean_date / 1000, title = "B",
                     repel = TRUE, shape = 21, pointsize = 2.9) +
  scale_fill_gradient2(name = "Mean age kyr BCE", low="yellow", mid="orange", 
                        high="red", midpoint = centdate) +
  theme(legend.position = "bottom") 
...
```{r supplementary3, fig.height = 15, fig.cap = "Same as above, only that here the proportion of non-flint is used as a supplementary column. A) Column plot, B) Row plot. The negligble difference between this CA and that above indicates that the flint/non-flint distinction is integrated in the different artefact types, and therefore that the effect of artefact types and raw material cannot be seperated. These plots thus hide important variability that is captured in the ones presented in the main text."}

# Version of the above where the proportion of non-flint is simply added as a 
# supplementary column, following suggestion from reviewer. 
moddat <- select(all_nonflint_prop, -site, -mean_date, -flint_count, 
                       -reported_start_bc, -reported_end_bc,
                       -askeladden_id)

nonpropsup <- CA(moddat, 
              col.sup = which(colnames(moddat) == "nonflint_prop"),  
                graph = FALSE)

# Seems to be a bug to change colour of supplementary column using 
# fviz_ca_row(): 
splt5 <- fviz(nonpropsup, element = "col", pointshape = 17,
              col.col.sup = "black",
             color = "contrib", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), repel = TRUE,
             title = "A") +
...
splt6 <- fviz_ca_row(nonpropsup, col.row = "black",
            fill = all_nonflint_prop$mean_date / 1000, title = "B",
                     repel = TRUE, shape = 21, pointsize = 2.9) +
  scale_fill_gradient2(name = "Mean age kyr BCE", low="yellow", mid="orange", 
                        high="red", midpoint = centdate) +
