...
```{r variable-aggregation, fig.cap = "Aggregation of variables for the correspondence analysis. Column on the left shows the data as originally compiled, the columns on the right shows how these have been aggregated for the analysis.", fig.height = 7}
...
```{r, fig.height = 15, fig.cap = "Same as above, only that the proportion of non-flint is used as a supplementary column. The negligble difference between the plots indicates that the raw material variability is captured by the different artefact types, and therefore that the effect of artefact types and raw material cannot be seperated. A) Row plot, B) Column plot."}
...
splt4 <- fviz_ca_col(nonpropsup, col.col = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), repel = TRUE,
             title = "B") +
  labs(colour = "Contribution to total inertia") +
  theme(legend.position = "bottom") 
...
```{r, fig.height = 18, fig.cap = "Correspondence analysis using all original artefact categories. A) Row plot, B) Column plot."}
all_cats <- sites %>%
  # Make site names the rownames
  column_to_rownames(var = "no") %>% 
  # Deselect a few of the columns not be used
